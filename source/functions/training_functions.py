...
def train(rank, EPOCHS, train_loader,model,
    ...
    perceptual_loss_function=PerceptualLoss().to(rank)
    ...
    if rank==0:
        ...
        perceptual_losses=[] if fraction==0 else torch.load('losses/perceptual_losses.pt')
    ...
    for epoch in range(EPOCHS):
        ...
        beta=1
        train_epoch(model, optimizer,train_loader,rank,perceptual_loss_function,beta)

        model.eval()
        with torch.no_grad():
            evaluate(model,train_loader,rank, perceptual_loss_function,beta,
                     reconstruction_losses=reconstruction_losses if rank==0 else None,
                     kl_divergences=kl_divergences if rank==0 else None,
                     perceptual_losses=perceptual_losses if rank==0 else None,total_losses=total_losses if rank==0 else None)
    ...
    if rank==0:
        ...
        if fraction!=total_fractions-1:
            ...
            torch.save(reconstruction_losses,'losses/perceptual_losses.pt')
            ...
            plot_losses(perceptual_losses,'perceptual_losses')
            ...
            if total_fractions>1:
                ...
                os.remove('losses/perceptual_losses.pt')
                os.remove('losses/total_losses.pt')

def train_epoch( model, optimizer,data_loader,device,perceptual_loss_function,beta=1):
    model.train()
    ...
    for _,data_point in enumerate(data_loader):
        ...
        loss=mse_loss(image,generated_image)+beta*beta_gaussian_kldiv(mu,log_var)+0.1*perceptual_loss_function(image,generated_image)
...
def evaluate(model,data_loader,device, perceptual_loss_function,beta=1,reconstruction_losses=None,kl_divergences=None,perceptual_losses=None,total_losses=None):
    total_loss=0
    reconstruction_loss=0
    kl_divergence=0
    monitored_perceptual_loss=0
    ...
    for _,data_point in enumerate(data_loader):
        ...
        single_perceptual_loss=perceptual_loss_function(image,generated_image)


        reconstruction_loss+=single_reconstruction_loss
        kl_divergence+=single_kl_divergence
        monitored_perceptual_loss+=single_perceptual_loss

        total_loss+=single_reconstruction_loss+beta*single_kl_divergence+0.1*single_perceptual_loss
    ...
    avg_perceptual_loss=monitored_perceptual_loss/len(data_loader)
    ...
    if total_losses is not None:
        ...
        perceptual_losses.append(avg_perceptual_loss.item())
